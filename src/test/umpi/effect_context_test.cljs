...
(deftest signal-build-context-test
...
(deftest effect-context-simple-test
  (async done
         (let [result (atom [])
               s1     (sig/signal 1)
...
(deftest effect-context-no-refs-test
  (is (nil? (ec/effect-context (fn [])))))


(deftest effect-context-batching-test
  (async done
         (let [result (atom [])
               s1     (sig/signal 1)
               f      (fn [] (swap! result conj @s1))
...
(deftest effect-context-updates-signal-watches-test
  (async done
         (let [result (atom [])
               s1     (sig/signal 0)
               s2     (sig/signal 's2)
